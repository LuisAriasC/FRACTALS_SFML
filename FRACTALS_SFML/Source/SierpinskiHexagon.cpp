...
void SierpinskiHexagon::Render(sf::RenderWindow& window) //OBTIENE UN RENDERWINDOW
{   /*ESTA FUNCIÓN OBTIENE LOS PARÁMETROS PARA ENCONTRAR LAS DIMENSIONES DE LOS PUNTOS
    ...
    drawSierpinskiHexagon(p1, center, radius, angle_radians, increment_radians, -1, window);
}

void SierpinskiHexagon::drawSierpinskiHexagon(const sf::Vector2f &reference_point,
    ...
    else
    {
        ...
        {
            ang_actual = 90.00 * (M_PI/180);
            drawSierpinskiHexagon(point0_0, center0, radius_3, ang_actual, incremento_ang, iteration+1, window);
            drawSierpinskiHexagon(point0_1, center1, radius_3, ang_actual, incremento_ang, iteration+1, window);
            drawSierpinskiHexagon(point0_2, center2, radius_3, ang_actual, incremento_ang, iteration+1, window);
            drawSierpinskiHexagon(point0_3, center3, radius_3, ang_actual, incremento_ang, iteration+1, window);
            drawSierpinskiHexagon(point0_4, center4, radius_3, ang_actual, incremento_ang, iteration+1, window);
            drawSierpinskiHexagon(point0_5, center5, radius_3, ang_actual, incremento_ang, iteration+1, window);
        }
    }
...
void SierpinskiHexagon::setNumberOfIterations(int newNumberOfIterations)
{
    m_numberOfIterations = newNumberOfIterations; //DA EL NÚMERO DE ITERACIONES
}

int SierpinskiHexagon::getNumberOfIterations() const
{
    return m_numberOfIterations; //OBTIENE EL NÚMERO DE ITERACIONES ***** BUSCAR DONDE SE USA
}

void SierpinskiHexagon::setBoundingBox(float x1, float y1, float x2, float y2)
...
sf::RectangleShape SierpinskiHexagon::getBoundingBox() const
{
    return m_boundingBox;                   //REGRESA EL BOUNDING BOX
}

void SierpinskiHexagon::setColor(const sf::Color& newColor) //DA EL COLOR PRINCIPAL ** VER SI SE USA OTRA VEZ
{
    m_color = sf::Color(newColor);
}

sf::Color SierpinskiHexagon::getColor() const //OBTIENE EL COLOR *** VER DONDE SE USA
{
    return m_color;
}
