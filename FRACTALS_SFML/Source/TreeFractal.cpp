...
void TreeFractal::drawTreeFractal(int iteration, const sf::Vector2f& rootPosition, float rootRotation, sf::RenderWindow& window)
{
    ...
    float x = rootPosition.x - sinf((rootRotation*M_PI)/180)*height;
    float y = rootPosition.y + cosl((rootRotation*M_PI)/180)*height;
...
void TreeFractal::Render(sf::RenderWindow& window)
{
    ...
    float x = m_startLocation.x - sinf((180*M_PI)/180) * m_initHeight;
    float y = m_startLocation.y + cosl((180*M_PI)/180) * m_initHeight;
